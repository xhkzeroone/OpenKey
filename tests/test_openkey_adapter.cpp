...
std::string typeSequence(openkey::OpenKeyAdapter &adapter,
  ...
  std::string rawAscii;
  for (char c : ascii) {
    std::string tempRaw = rawAscii;
    tempRaw.push_back(c);
    std::string normalizedRaw = adapter.normalizeRawKeystrokes(tempRaw);
    if (normalizedRaw != tempRaw) {
      word = adapter.convertRawBuffer(normalizedRaw);
      rawAscii = std::move(normalizedRaw);
    } else {
...
std::string simulateBackspaceSnapshot(openkey::OpenKeyAdapter &adapter,
  ...
  for (char c : ascii) {
    ...
    if (c == '<') {
      if (canReseedFromSnapshot && shown.empty() && !snapshotShown.empty()) {
        shown = snapshotShown;
    ...
    const unsigned int deleteCount = utf8CharCount(shown.substr(prefixLen));
    const std::string commitText = r.newWord.substr(prefixLen);

    if (deleteCount == 0) {
      appText += commitText;
      out << "key " << c << ": commit " << quote(commitText)
          << " -> app=" << quote(appText) << " shown=" << quote(r.newWord)
          << "\n";
    } else {
...
int main() {
  ...
  expectEq("dduwocj", typeSequence(adapter, "dduwocj"), "được");
  {
    ...
    if (restoreAdapter.restoreFromRawAsciiOnWordBreak(shown, "pussh", ' ',
                                                      restored)) {
      failures++;
      std::cerr << "[FAIL] restoreFromRaw pussh: expected false, got true "
                   "(restored to: "
                << restored << ")\n";
    }
  }
  {
    ...
    if (restoreAdapter.restoreFromRawAsciiOnWordBreak(shown, "wwass", ' ',
                                                      restored)) {
      failures++;
      std::cerr << "[FAIL] restoreFromRaw wwass: expected false, got true "
                   "(restored to: "
                << restored << ")\n";
    }
  }
  {
    ...
    if (restoreAdapter.restoreFromRawAsciiOnWordBreak(shown, "thorr", ' ',
                                                      restored)) {
      failures++;
      std::cerr << "[FAIL] restoreFromRaw thorr: expected false, got true "
                   "(restored to: "
                << restored << ")\n";
    }
  }
  {
    ...
    if (restoreAdapter.restoreFromRawAsciiOnWordBreak(shown, "mass", ' ',
                                                      restored)) {
      failures++;
      std::cerr << "[FAIL] restoreFromRaw mass: expected false, got true "
                   "(restored to: "
                << restored << ")\n";
    }
  }
  {
    auto restoreAdapter = makeAdapter();
    restoreAdapter.setLiteralWAtWordStart(true);
    std::string restored;
  ...
  }
  {
    auto restoreAdapter = makeAdapter();
    std::string restored;
    if (restoreAdapter.restoreFromRawAsciiOnWordBreak("w", "ww", ' ',
                                                      restored)) {
      failures++;
      std::cerr << "[FAIL] restoreFromRaw ww with literal W off: expected false, "
                   "got true (restored to: "
                << restored << ")\n";
    }
    if (restoreAdapter.restoreFromRawAsciiOnWordBreak("ư", "ww", ' ',
                                                      restored)) {
      failures++;
      std::cerr << "[FAIL] restoreFromRaw ư with literal W off: expected false, "
                   "got true (restored to: "
                << restored << ")\n";
    }
  }
  ...
  }
  {
    ...
    expectEq("nonPreedit snapshot",
             ...
             "key s: helper PLAN backspaces=1; helper DONE; commit \"á\" -> "
             "app=\"á\" shown=\"á\"\n"
             "final app=\"á\" shown=\"á\"");
  }
  {
    ...
    expectEq("nonPreedit reseed snapshot",
             ...
             "key f: helper PLAN backspaces=1; helper DONE; commit \"à\" -> "
             "app=\"à\" shown=\"à\"\n"
             "final app=\"à\" shown=\"à\"");
  }
  ...
  // Test normalizeRawKeystrokes
  {
    expectEq("normalize raw uwoc", adapter.normalizeRawKeystrokes("uwoc"),
             "uowc");
    expectEq("normalize raw uwo", adapter.normalizeRawKeystrokes("uwo"), "uow");
    expectEq("normalize raw uwoj", adapter.normalizeRawKeystrokes("uwoj"),
             "uowj");
    expectEq("normalize raw duwowng", adapter.normalizeRawKeystrokes("duwowng"),
             "duowwng");
    expectEq("normalize raw dduwocj", adapter.normalizeRawKeystrokes("dduwocj"),
             "dduowcj");
    expectEq("normalize raw nguwoif", adapter.normalizeRawKeystrokes("nguwoif"),
             "nguowif");
    expectEq("normalize raw owanf", adapter.normalizeRawKeystrokes("owanf"),
             "oawnf");
    expectEq("normalize raw oawnf", adapter.normalizeRawKeystrokes("oawnf"),
             "oawnf");

    // English words
    expectEq("normalize raw network", adapter.normalizeRawKeystrokes("network"),
             "network");
    expectEq("normalize raw weekend", adapter.normalizeRawKeystrokes("weekend"),
             "weekend");
    expectEq("normalize raw sweet", adapter.normalizeRawKeystrokes("sweet"),
             "sweet");
    expectEq("normalize raw news", adapter.normalizeRawKeystrokes("news"),
             "news");
    expectEq("normalize raw backspace",
             adapter.normalizeRawKeystrokes("backspace"), "backspace");
    expectEq("normalize raw display", adapter.normalizeRawKeystrokes("display"),
             "display");
    expectEq("normalize raw sport", adapter.normalizeRawKeystrokes("sport"),
             "sport");
    expectEq("normalize raw Iowa", adapter.normalizeRawKeystrokes("Iowa"),
             "Iowa");
    expectEq("normalize raw Rowan", adapter.normalizeRawKeystrokes("Rowan"),
             "Rowan");
    expectEq("normalize raw Bowan", adapter.normalizeRawKeystrokes("Bowan"),
             "Bowan");
    expectEq("normalize raw twoway", adapter.normalizeRawKeystrokes("twoway"),
             "twoway");
    expectEq("normalize raw duong", adapter.normalizeRawKeystrokes("dduwowfng"),
             "dduowwfng");
    expectEq("normalize raw truong",
             adapter.normalizeRawKeystrokes("truwowfng"), "truowwfng");
    expectEq("normalize raw nguoi",
             adapter.normalizeRawKeystrokes("nguwowif"), "nguowwif");
    expectEq("normalize raw khoan",
             adapter.normalizeRawKeystrokes("khowanf"), "khoawnf");
    expectEq("normalize raw xoan", adapter.normalizeRawKeystrokes("xowanf"),
             "xoawnf");
    expectEq("normalize raw loan", adapter.normalizeRawKeystrokes("lowanf"),
             "loawnf");
